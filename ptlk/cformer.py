""" Cformer - 轻量级收集分发Transformer v1.0
    基于Collect-and-Distribute机制的点云特征提取器
    
    设计特点:
    1. 双阶段注意力 - 先捕获局部特征，再整合全局上下文
    2. 收集分发机制 - 通过代理点收集并分发全局信息
    3. 计算效率高 - 降低注意力计算复杂度
    4. 保持API兼容 - 与PointNet_features完全兼容
"""
...
class CFormer_features(torch.nn.Module):
    ...
    采用collect-and-distribute transformer架构，高效处理点云特征
    输入: [B, N, 3] 点云
    输出: [B, K] 全局特征向量
    """
