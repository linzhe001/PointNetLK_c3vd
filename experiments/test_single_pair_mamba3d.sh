...
#$ -l gpu=true
#$ -pe gpu 1
#$ -N ljiang_test_single_pair_mamba3d_depth_mesh
#$ -o /SAN/medic/MRpcr/logs/f_test_single_pair_mamba3d_depth_mesh_output.log
#$ -e /SAN/medic/MRpcr/logs/f_test_single_pair_mamba3d_depth_mesh_error.log
#$ -wd /SAN/medic/MRpcr

# Mamba3D单对点云配准测试脚本 - 多体素化参数测试
# Mamba3D Single pair point cloud registration test script - Multiple voxelization parameters test

echo "========== Mamba3D单对点云配准测试 - 多体素化参数 Mamba3D Single Pair Point Cloud Registration Test - Multiple Voxelization Parameters =========="
...
# Mamba3D模型文件路径
MODEL_PATH="/SAN/medic/MRpcr/results/mamba3d_c3vd/mamba3d_pointlk_resume_0620_model_best.pth"

# 检查模型文件是否存在
if [ ! -f "${MODEL_PATH}" ]; then
    echo "错误: Mamba3D模型文件不存在: ${MODEL_PATH}"
    echo "请确保模型文件路径正确或训练相应的模型"
    exit 1
fi

# 输出目录
OUTPUT_DIR="/SAN/medic/MRpcr/test_results_output/single_pair_mamba3d_test_depth_mesh"
mkdir -p ${OUTPUT_DIR}
...
echo "========== Mamba3D配置信息 Mamba3D Configuration =========="
...
for voxel_name in "${!VOXEL_CONFIGS[@]}"; do
    ...
    for i in "${!PERTURBATIONS[@]}"; do
        ...
        OUTPUT_FILE="${OUTPUT_DIR}/mamba3d_${voxel_name}_result_${i}_$(echo ${PERTURBATION} | tr ',' '_').csv"
...
echo ""
echo "========== Mamba3D多体素化参数测试完成 Mamba3D Multiple Voxelization Parameters Tests Completed =========="
echo "所有Mamba3D结果保存在: ${OUTPUT_DIR}"
echo "查看结果文件:"
ls -la ${OUTPUT_DIR}/*.csv 2>/dev/null || echo "没有生成结果文件"

# 生成测试总结
SUMMARY_FILE="${OUTPUT_DIR}/mamba3d_voxel_test_summary_$(date +%Y%m%d_%H%M%S).txt"
echo "Mamba3D多体素化参数测试总结" > ${SUMMARY_FILE}
echo "==============================" >> ${SUMMARY_FILE}
...
echo "" >> ${SUMMARY_FILE}
echo "Mamba3D模型参数:" >> ${SUMMARY_FILE}
echo "特征维度: ${DIM_K}" >> ${SUMMARY_FILE}
echo "Mamba块数量: ${NUM_MAMBA_BLOCKS}" >> ${SUMMARY_FILE}
echo "状态空间维度: ${D_STATE}" >> ${SUMMARY_FILE}
echo "扩展因子: ${EXPAND}" >> ${SUMMARY_FILE}
echo "聚合函数: ${SYMFN}" >> ${SUMMARY_FILE}
echo "" >> ${SUMMARY_FILE}
