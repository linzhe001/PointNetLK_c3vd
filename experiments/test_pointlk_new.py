...
def options(argv=None):
    ...
    parser.add_argument('--dataset-type', default='modelnet', choices=['modelnet', 'mydata'],
        ...
        with open(self.filename, 'w') as fout:
            ...
            with torch.no_grad():
                for i, data in enumerate(testloader):
                    ...
                    rot_error = dx[:, :3]  # rotation error [1, 3]
                    trans_error = dx[:, 3:]  # translation error [1, 3]

                    rot_norm = rot_error.norm(p=2, dim=1)  # L2 Norm
                    trans_norm = trans_error.norm(p=2, dim=1)  # L2 Norm                
                    #dn = dx.norm(p=2, dim=1) # --> [1]
                    #dm = dn.mean()
                    self.eval_1__write(fout, ig_gt, g_hat)
                    LOGGER.info('test, %d/%d, rot_error: %f, trans_error: %f', i, len(testloader), rot_norm.item(), trans_norm.item())
