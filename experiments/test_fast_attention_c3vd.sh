...
# Fast Point Attention model configuration (keep consistent with training)
DIM_K=1024                    # Feature dimension
NUM_FAST_ATTENTION_BLOCKS=2   # Number of fast attention blocks
FAST_ATTENTION_SCALE=1        # Model scale factor (1=standard, 2=light, 4=ultra-light)
SYMFN="max"                   # Aggregation function: max, avg, or selective
...
FAST_ATTENTION_MODEL_PREFIX="/SAN/medic/MRpcr/results/fast_attention_c3vd/fast_attention_pointlk_${DATE_TAG}"
FAST_ATTENTION_MODEL="${FAST_ATTENTION_MODEL_PREFIX}_model_best.pth"
...
if [ ! -f "${FAST_ATTENTION_MODEL}" ]; then
    echo "⚠️  Today's model file not found: ${FAST_ATTENTION_MODEL}"
    echo "🔍 Searching for latest fast attention model..."
    
    # Search for latest fast attention pointlk model
    LATEST_MODEL=$(find /SAN/medic/MRpcr/results/fast_attention_c3vd/ -name "fast_attention_pointlk_*_model_best.pth" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
    
    if [ -n "${LATEST_MODEL}" ] && [ -f "${LATEST_MODEL}" ]; then
        FAST_ATTENTION_MODEL="${LATEST_MODEL}"
        # Extract model prefix
        FAST_ATTENTION_MODEL_PREFIX=$(echo "${FAST_ATTENTION_MODEL}" | sed 's/_model_best\.pth$//')
        echo "✅ Found latest model: ${FAST_ATTENTION_MODEL}"
    else
        echo "❌ Error: No fast attention model files found!"
        echo "Please ensure train_fast_attention_c3vd.sh has been run and successfully trained the model"
        exit 1
    fi
fi
...
echo "========== Fast Point Attention Registration Model Test Configuration =========="
echo "🧠 Model type: Fast Point Attention registration model"
...
echo ""
echo "🔧 Fast Point Attention parameters:"
echo "   - Feature dimension: ${DIM_K}"
echo "   - Number of fast attention blocks: ${NUM_FAST_ATTENTION_BLOCKS}"
echo "   - Model scale factor: ${FAST_ATTENTION_SCALE}"
echo "   - Aggregation function: ${SYMFN}"
echo "   - LK max iterations: ${MAX_ITER}"
echo "   - LK step size: ${DELTA}"
echo ""
echo "📁 Model files:"
echo "   - Registration model: ${FAST_ATTENTION_MODEL}"
...
# Check model file
if [ -f "${FAST_ATTENTION_MODEL}" ]; then
    echo "✅ Fast Point Attention registration model exists: ${FAST_ATTENTION_MODEL}"
    MODEL_SIZE=$(du -h "${FAST_ATTENTION_MODEL}" | cut -f1)
    echo "📊 Model file size: ${MODEL_SIZE}"
else
    echo "❌ Error: Fast Point Attention registration model does not exist: ${FAST_ATTENTION_MODEL}"
    exit 1
fi
...
echo ""
echo "========== Starting Fast Point Attention Registration Model Test =========="
...
${PY3} test_pointlk.py \
  ...
  --symfn ${SYMFN} \
  --pretrained ${FAST_ATTENTION_MODEL} \
  ${VISUALIZE_PARAMS}

# Check test results
if [ $? -eq 0 ]; then
    echo ""
    echo "🎉 Fast Point Attention registration model test completed!"
    ...
    echo ""
    echo "❌ Fast Point Attention registration model test failed!"
...
CONFIG_FILE="${TEST_RESULTS_DIR}/fast_attention_test_${DATE_TAG}_config.txt"
echo "🧠 Fast Point Attention Registration Model Test Configuration" > ${CONFIG_FILE}
echo "=====================================" >> ${CONFIG_FILE}
echo "Test completion time: $(date)" >> ${CONFIG_FILE}
echo "" >> ${CONFIG_FILE}
echo "🔧 Model configuration:" >> ${CONFIG_FILE}
echo "Model type: Fast Point Attention registration model" >> ${CONFIG_FILE}
echo "Feature dimension: ${DIM_K}" >> ${CONFIG_FILE}
echo "Number of fast attention blocks: ${NUM_FAST_ATTENTION_BLOCKS}" >> ${CONFIG_FILE}
echo "Model scale factor: ${FAST_ATTENTION_SCALE}" >> ${CONFIG_FILE}
...
echo "Registration model: ${FAST_ATTENTION_MODEL}" >> ${CONFIG_FILE}
echo "" >> ${CONFIG_FILE}
...
echo ""
echo "🎉🎉🎉 Fast Point Attention registration model test all completed! 🎉🎉🎉" 