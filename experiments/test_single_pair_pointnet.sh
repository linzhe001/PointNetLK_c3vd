...
#$ -l gpu=true
#$ -pe gpu 1
#$ -N ljiang_test_single_pair_pointnet_depth_mesh
#$ -o /SAN/medic/MRpcr/logs/f_test_single_pair_pointnet_depth_mesh_output.log
#$ -e /SAN/medic/MRpcr/logs/f_test_single_pair_pointnet_depth_mesh_error.log
#$ -wd /SAN/medic/MRpcr

# PointNet单对点云配准测试脚本 - 多体素化参数测试
# PointNet Single pair point cloud registration test script - Multiple voxelization parameters test

echo "========== PointNet单对点云配准测试 - 多体素化参数 PointNet Single Pair Point Cloud Registration Test - Multiple Voxelization Parameters =========="
...
# PointNet模型文件路径
MODEL_PATH="/SAN/medic/MRpcr/results/c3vd_augmented/c3vd_pointlk_all_0407_model_best.pth"

# 检查模型文件是否存在
if [ ! -f "${MODEL_PATH}" ]; then
    echo "错误: PointNet模型文件不存在: ${MODEL_PATH}"
    echo "请确保模型文件路径正确或训练相应的模型"
    exit 1
fi

# 输出目录
OUTPUT_DIR="/SAN/medic/MRpcr/test_results_output/single_pair_pointnet_test_depth_mesh"
mkdir -p ${OUTPUT_DIR}
...
echo "========== PointNet配置信息 PointNet Configuration =========="
...
for voxel_name in "${!VOXEL_CONFIGS[@]}"; do
    ...
    for i in "${!PERTURBATIONS[@]}"; do
        ...
        OUTPUT_FILE="${OUTPUT_DIR}/pointnet_${voxel_name}_result_${i}_$(echo ${PERTURBATION} | tr ',' '_').csv"
...
echo ""
echo "========== PointNet多体素化参数测试完成 PointNet Multiple Voxelization Parameters Tests Completed =========="
echo "所有PointNet结果保存在: ${OUTPUT_DIR}"
echo "查看结果文件:"
ls -la ${OUTPUT_DIR}/*.csv 2>/dev/null || echo "没有生成结果文件"

# 生成测试总结
SUMMARY_FILE="${OUTPUT_DIR}/pointnet_voxel_test_summary_$(date +%Y%m%d_%H%M%S).txt"
echo "PointNet多体素化参数测试总结" > ${SUMMARY_FILE}
echo "==============================" >> ${SUMMARY_FILE}
