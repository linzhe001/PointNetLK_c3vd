...
#$ -l gpu_type=a6000
#$ -pe gpu 1
#$ -N ljiang_train_mamba_v4_cbam_pointlk_loss2
#$ -o /SAN/medic/MRpcr/logs/mamba_v4_cbam_pointlk_c3vd_loss2_output.log
#$ -e /SAN/medic/MRpcr/logs/mamba_v4_cbam_pointlk_c3vd_loss2_error.log
#$ -wd /SAN/medic/MRpcr

#=============================================================================
# Mamba3D-v4 (CBAM) C3VD PointLK 配准模型训练脚本 (双损失+全局特征约束)
#=============================================================================
#
# 此脚本使用预训练的 Mamba3D-v4 分类器权重，训练一个不使用领域适应的
...
# 自动寻找最新的分类器权重
echo "正在寻找最新的Mamba3D-v4 (CBAM)分类器权重..."
CLASSIFIER_RESULT_DIR="/SAN/medic/MRpcr/results/mamba_v4_cbam_c3vd"
LATEST_FEAT_FILE=$(find_latest_classifier "$CLASSIFIER_RESULT_DIR" "mamba_v4_cbam_classifier")

if [ -z "$LATEST_FEAT_FILE" ]; then
    echo "错误: 在 $CLASSIFIER_RESULT_DIR 中未找到任何 mamba_v4_cbam_classifier_*_feat_best.pth 文件"
    echo "请先运行分类器训练脚本: train_mamba_v4_cbam_c3vd.sh"
    exit 1
fi
...
# --- 打印配置信息 ---
echo "========== Mamba3D-v4 (CBAM) C3VD PointLK训练配置 =========="
echo "模型: Mamba3D-v4 (CBAM) PointLK (双损失+全局特征约束)"
...
# =============================================
echo "========== 训练Mamba3D-v4 (CBAM) PointLK =========="
POINTLK_PREFIX="/SAN/medic/MRpcr/results/mamba_v4_cbam_c3vd/mamba_v4_cbam_pointlk_${DATE_TAG}_loss2"
