...
#$ -l gpu_type=a6000
#$ -pe gpu 1
#$ -N ljiang_train_cformer_modelnet
#$ -o /SAN/medic/MRpcr/logs/f_cformer_modelnet_output.log
#$ -e /SAN/medic/MRpcr/logs/f_cformer_modelnet_error.log
#$ -wd /SAN/medic/MRpcr
...
# 创建结果和日志目录
mkdir -p /SAN/medic/MRpcr/results/cformer_modelnet
...
# 定义输出路径
CLASSIFIER_PREFIX="/SAN/medic/MRpcr/results/cformer_modelnet/cformer_classifier_${DATE_TAG}"
POINTLK_PREFIX="/SAN/medic/MRpcr/results/cformer_modelnet/cformer_pointlk_${DATE_TAG}"
CLASSIFIER_LOG="${CLASSIFIER_PREFIX}.log"
POINTLK_LOG="${POINTLK_PREFIX}.log"

# 打印配置信息
echo "========== CFormer 两阶段训练配置 =========="
echo "🧠 模型类型: CFormer (两阶段训练)"
...
echo ""
echo "========== CFormer内存优势分析 =========="
echo "💡 CFormer采用收集分发机制 (Collect-and-Distribute)，相比标准Transformer更高效:"
echo "   - 通过代理点降低注意力计算复杂度"
echo "   - 双阶段注意力流程捕获局部和全局特征"
echo "   - 训练速度通常比标准Transformer更快"
echo "   - 计算资源需求更低"

# =============================================
# 第一阶段：训练CFormer分类器
# =============================================
echo "========== 第一阶段：训练CFormer分类器 =========="
echo "🧠 开始训练CFormer分类器..."
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "❌ 第一阶段：CFormer分类器训练失败!"
    echo "请检查错误日志: ${CLASSIFIER_LOG}"
    exit 1
fi

# =============================================
# 第二阶段：训练CFormer-PointLK配准模型
# =============================================
echo ""
echo "========== 第二阶段：训练CFormer-PointLK配准模型 =========="
echo "🧠 开始训练CFormer-PointLK配准模型..."
...
if [ $? -eq 0 ]; then
    echo ""
    echo "🎉 第二阶段：CFormer-PointLK配准训练完成!"
    ...
    echo ""
    echo "❌ 第二阶段：CFormer-PointLK配准训练失败!"
...
# =============================================
echo ""
echo "🎉🎉🎉 ModelNet上的CFormer两阶段训练全部完成! 🎉🎉🎉"
echo ""
echo "📊 训练总结:"
echo "✅ 第一阶段：CFormer分类器训练成功"
echo "   📁 分类器模型: ${CLASSIFIER_PREFIX}_*.pth"
echo "   📋 分类器日志: ${CLASSIFIER_LOG}"
echo ""
echo "✅ 第二阶段：CFormer-PointLK配准训练成功"
...
# 保存详细配置信息
CONFIG_FILE="/SAN/medic/MRpcr/results/cformer_modelnet/cformer_modelnet_${DATE_TAG}_config.txt"
echo "🧠 CFormer ModelNet训练配置信息" > ${CONFIG_FILE}
echo "=====================================" >> ${CONFIG_FILE}
echo "训练完成时间: $(date)" >> ${CONFIG_FILE}
echo "" >> ${CONFIG_FILE}
echo "🔧 模型配置:" >> ${CONFIG_FILE}
echo "模型类型: CFormer (两阶段训练)" >> ${CONFIG_FILE}
...
echo ""
echo "🎯 最终结果文件:"
echo "📂 结果目录: /SAN/medic/MRpcr/results/cformer_modelnet/"
