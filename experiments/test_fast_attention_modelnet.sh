...
#$ -l gpu_type=a6000
#$ -pe gpu 1
#$ -N ljiang_test_fast_attention_modelnet
#$ -o /SAN/medic/MRpcr/logs/f_test_fast_attention_modelnet_output.log
#$ -e /SAN/medic/MRpcr/logs/f_test_fast_attention_modelnet_error.log
#$ -wd /SAN/medic/MRpcr
...
# 创建结果和日志目录
mkdir -p /SAN/medic/MRpcr/results/fast_attention_modelnet/test_results
...
# Fast Point Attention模型配置 (与训练保持一致)
DIM_K=1024                    # 特征维度
...
# 模型路径 (需要根据实际训练结果调整)
FAST_ATTENTION_MODEL_PREFIX="/SAN/medic/MRpcr/results/fast_attention_modelnet/fast_attention_pointlk_${DATE_TAG}"
...
if [ ! -f "${FAST_ATTENTION_MODEL}" ]; then
    echo "⚠️  未找到今天的模型文件: ${FAST_ATTENTION_MODEL}"
    echo "🔍 搜索最新的fast attention模型..."
    
    # 搜索最新的fast attention pointlk模型
    LATEST_MODEL=$(find /SAN/medic/MRpcr/results/fast_attention_modelnet/ -name "fast_attention_pointlk_*_model_best.pth" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
    ...
    if [ -n "${LATEST_MODEL}" ] && [ -f "${LATEST_MODEL}" ]; then
        ...
        echo "✅ 找到最新模型: ${FAST_ATTENTION_MODEL}"
    else
        echo "❌ 错误: 未找到任何fast attention模型文件!"
        echo "请确保已运行train_fast_attention_modelnet.sh并成功训练了模型"
        exit 1
    fi
fi
...
# 打印配置信息
echo "========== Fast Point Attention配准模型测试配置 =========="
echo "🧠 模型类型: Fast Point Attention配准模型"
...
echo ""
echo "📁 模型文件:"
echo "   - 配准模型: ${FAST_ATTENTION_MODEL}"
...
# 检查模型文件
if [ -f "${FAST_ATTENTION_MODEL}" ]; then
    echo "✅ Fast Point Attention配准模型存在: ${FAST_ATTENTION_MODEL}"
    MODEL_SIZE=$(du -h "${FAST_ATTENTION_MODEL}" | cut -f1)
    echo "📊 模型文件大小: ${MODEL_SIZE}"
else
    echo "❌ 错误: Fast Point Attention配准模型不存在: ${FAST_ATTENTION_MODEL}"
    exit 1
fi
...
echo ""
echo "========== 开始Fast Point Attention配准模型测试 =========="
echo "🚀 即将开始测试..."
...
if [ $? -eq 0 ]; then
    echo ""
    echo "🎉 Fast Point Attention配准模型测试完成!"
    ...
    echo ""
    echo "❌ Fast Point Attention配准模型测试失败!"
...
echo "🧠 Fast Point Attention配准模型测试配置" > ${CONFIG_FILE}
echo "=====================================" >> ${CONFIG_FILE}
echo "测试完成时间: $(date)" >> ${CONFIG_FILE}
echo "" >> ${CONFIG_FILE}
echo "🔧 模型配置:" >> ${CONFIG_FILE}
echo "模型类型: Fast Point Attention配准模型" >> ${CONFIG_FILE}
...
echo "配准模型: ${FAST_ATTENTION_MODEL}" >> ${CONFIG_FILE}
echo "" >> ${CONFIG_FILE}
...
echo ""
echo "🎉🎉🎉 Fast Point Attention配准模型测试全部完成! 🎉🎉🎉" 