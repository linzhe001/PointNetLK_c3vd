...
# 创建结果和日志目录
mkdir -p /SAN/medic/MRpcr/results/pointnet_c3vd/test_results
mkdir -p /SAN/medic/MRpcr/results/pointnet_c3vd/test_results/gt
...
if [ -f "${POINTNET_MODEL}" ]; then
    ...
    echo "✅ 找到指定的PointNet配准模型: ${POINTNET_MODEL}"
else
    echo "❌ 错误: 指定的PointNet配准模型文件不存在: ${POINTNET_MODEL}"
    echo "请检查模型文件路径是否正确"
    exit 1
fi
...
# 检查模型文件是否存在
if [ ! -f "${POINTNET_MODEL}" ]; then
    echo "❌ 错误: PointNet配准模型文件不存在: ${POINTNET_MODEL}"
    echo "请确保模型文件路径正确"
    exit 1
else
    echo "✅ 使用PointNet配准模型: ${POINTNET_MODEL}"
    MODEL_SIZE=$(du -h "${POINTNET_MODEL}" | cut -f1)
    echo "📊 配准模型文件大小: ${MODEL_SIZE}"
fi
...
# 测试结果输出目录
TEST_RESULTS_DIR="/SAN/medic/MRpcr/results/pointnet_c3vd/test_results"
...
echo ""
echo "========== 第二轮测试：GT姿态文件 =========="
echo "🎯 测试目标: 处理单独的 gt_poses.csv 文件"
...
echo "   ├── angle_000/ (第一轮：0度扰动)"
echo "   ├── angle_010/ (第一轮：10度扰动)"
echo "   ├── ..."
echo "   ├── angle_090/ (第一轮：90度扰动)"
echo "   ├── gt/        (第二轮：GT姿态测试)"
